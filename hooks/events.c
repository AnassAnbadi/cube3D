...
/*   Updated: 2025/09/23 15:23:37 by mbenchah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mouse_move(int x, int y, t_data *data)
{
	...
	old_normal_x = data->normal.x;
	data->normal.x = data->normal.x * cos(angle) - data->normal.y * sin(angle);
	data->normal.y = old_normal_x * sin(angle) + data->normal.y * cos(angle);
...
int	move_(t_data *data)
{
	...
	if (data->key_fleche == LEFT)
	{
		...
		data->normal.x = data->normal.x * cos(ROT) + data->normal.y * sin(ROT);
		data->normal.y = -old_normal_x * sin(ROT) + data->normal.y * cos(ROT);
	}
