...
void	parse_texture_line(t_data *data, char *line)
{
	...
	{
		get_value()[0]++;
		data->config.north_texture = ft_get_value(line + 3, 1);
	}
	else if (ft_strncmp(line, "SO ", 3) == 0)
	{
		get_value()[1]++;
		data->config.south_texture = ft_get_value(line + 3, 1);
	}
	else if (ft_strncmp(line, "EA ", 3) == 0)
	{
		get_value()[2]++;
		data->config.east_texture = ft_get_value(line + 3, 1);
	}
	else if (ft_strncmp(line, "WE ", 3) == 0)
	{
		get_value()[3]++;
		data->config.west_texture = ft_get_value(line + 3, 1);
	}
}
